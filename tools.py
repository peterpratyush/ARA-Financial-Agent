...
# 1. Initialize the embedding model (runs locally, free)
embeddings = HuggingFaceEmbeddings(model_name="sentence-transformers/all-MiniLM-L6-v2")

# 2. Initialize ChromaDB
chroma_client = chromadb.PersistentClient(path="./vector_db")
collection = chroma_client.get_or_create_collection(name="financial_memory")

def retrieve_financial_context(query: str, ticker: str, top_k: int = 5):
    """
    Retrieves context using local embeddings compatible with Groq-based agents.
    """
    try:
        # Convert query to vector using the local model
